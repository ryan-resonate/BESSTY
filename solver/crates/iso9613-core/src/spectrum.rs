//! Band-system support: octave (10 bands, 16 Hz – 8 kHz) and one-third octave
//! (31 bands, 10 Hz – 10 kHz).
//!
...
//! `BandSpectrum` carries one `f64` value per band.
//!
//! NOTE (non-normative extension): ISO 9613-2 defines the method for the
//! eight octave bands 63 Hz – 8 kHz. The 16 / 31.5 Hz octaves and the
//! third-octave system are implementation extensions — sub-63 Hz bands use
//! the 63 Hz Table-3 coefficients (see `ground::general`), and third-octave
//! bands inherit their parent octave's ground coefficients.
...
impl BandSystem {
    ...
    /// Octave index (0..10) for ground-attenuation Table 3 lookup.
...
impl BandSpectrum {
    pub fn zeros(system: BandSystem) -> Self {
        ...
        for _ in 0..n { bands.push(0.0); }
        Self { system, bands }
    }

    pub fn from_iter<I: IntoIterator<Item = f64>>(system: BandSystem, iter: I) -> Self {
        let bands: SmallVec<[f64; 32]> = iter.into_iter().collect();
        assert_eq!(
...
mod tests {
    ...
    fn parent_octave_mapping() {
        ...
        // 1000 Hz third-octave is at index 20 in the 31-band list and parent
        // octave 6 (1k is the 7th entry in the 10-band octave list).
