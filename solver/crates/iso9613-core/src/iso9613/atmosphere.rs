//! ISO 9613-2 — 7.2 Atmospheric absorption (1996 Eq 8 ≡ 2024 Eq 9), with the
//! per-band coefficient α(f, T, h, p) computed from first principles per
//! ISO 9613-1:1996 §8 + Annex E.
//!
//! `Aatm = αatm · d / 1000`, with `αatm` in dB/km.
//!
//! Edition note: the formula is identical in both editions; they differ only
//! in where α comes from (1996 tabulates values in its Table 2; 2024 mandates
//! the ISO 9613-1 computation at exact centre frequencies). This module is
//! the computed source; a 1996 Table-2 lookup option is added with the
//! edition switch if the TR 17534-3 step values require it.
