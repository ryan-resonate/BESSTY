...
use beesty_solver::iso9613::barrier::{BarrierConvention, LateralEdge, WallBarrier};
use beesty_solver::iso9613::{annex_d, evaluate_free_field, evaluate_with_barriers, evaluate_with_ground};
...
fn flat_lw(system: BandSystem, level: f64) -> BandSpectrum<f64> {
    BandSpectrum::from_iter(system, std::iter::repeat(level).take(system.n_bands()))
}
...
fn run_all(out: &mut Vec<String>) {
    ...
    let mut push = |label: &str, lp: &BandSpectrum<f64>| {
