...
fn time_one_eval(walls: &[WallBarrier<f64>], iters: u32) -> f64 {
    let lw = flat_100();
...
fn subdivided_barrier(pieces: usize) -> Vec<WallBarrier<f64>> {
    (0..pieces)
        .map(|k| {
...
fn bench_grid_projection() {
    let no_bar: Vec<WallBarrier<f64>> = vec![];
