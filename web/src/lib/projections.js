...
const MGA94 = [49, 50, 51, 52, 53, 54, 55, 56].map((zone) => ({
    code: 28300 + zone,
...
const MGA2020 = [49, 50, 51, 52, 53, 54, 55, 56].map((zone) => ({
    code: 7849 + (zone - 49),
...
const PRESET_BY_CODE = new Map(ALL_PRESETS.map((p) => [p.code, p]));
...
export function listEpsgPresets() {
    return ALL_PRESETS;
}
/// Group by `group` field for `<optgroup>` rendering.
export function groupedEpsgPresets() {
    const seen = new Map();
    for (const p of ALL_PRESETS) {
...
export function presetForEpsg(code) {
...
export function registerCustomEpsg(code, defn, label) {
    if (PRESET_BY_CODE.has(code))
        return;
...
/// `presetForEpsg`).
export function toWgs84(epsg, x, y) {
    if (epsg === 4326)
        return [y, x]; // proj4 input order is (x, y) = (lng, lat).
    const def = `EPSG:${epsg}`;
...
export function fromWgs84(epsg, lat, lng) {
    if (epsg === 4326)
        return [lng, lat];
    const def = `EPSG:${epsg}`;
...
/// `registerCustomEpsg`).
export function isSupportedEpsg(code) {
