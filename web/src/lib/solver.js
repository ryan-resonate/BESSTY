...
//      drag settles.
import init, { evaluate_general_octave, evaluate_wtg_octave, evaluate_general_with_grad_src_octave, evaluate_wtg_with_grad_src_octave, octave_a_weighting, octave_centres, } from '../wasm/beesty_solver.js';
...
function aWeights(bs) {
    return bs === 'oneThirdOctave' ? THIRD_OCT_AW : OCTAVE_AW;
}
let initialized = null;
export function ensureSolverReady() {
    if (!initialized)
        initialized = init().then(() => undefined);
    return initialized;
}
function packBarriers(barriers, originLatLng) {
    const out = [];
    for (const b of barriers) {
        if (b.polylineLatLng.length < 2)
            continue;
...
export function latLngToLocalMetres(latLng, origin) {
    const R = 6371008.8;
...
function sourceAbsZ(source, project, dem) {
...
function snapshotPair(source, rxLatLng, rxHeightAboveGround, project, barriersFlat, dem, origin) {
    ...
    if (source.kind === 'wtg') {
        ...
        const snap = evaluate_wtg_with_grad_src_octave(lw, se, sn, hubZ, re, rn, rxZ, g, barriersFlat, entry.rotorDiameterM ?? 120, false);
        return { snapshot: snap, srcAbsXyz: [se, sn, hubZ] };
    }
    const sourceZ = groundSrc + (source.elevationOffset ?? 0) + 1.5;
    const snap = evaluate_general_with_grad_src_octave(lw, se, sn, sourceZ, re, rn, rxZ, g, barriersFlat);
...
function extrapolateLpClamped(snapshot, srcAbsAtSnapshot, srcAbsNow, capPerBandDb) {
...
function aWeightedTotal(perBandLp, aw) {
    let aSum = 0;
    const n = Math.min(perBandLp.length, aw.length);
    for (let i = 0; i < n; i++) {
        if (isFinite(perBandLp[i]))
            aSum += Math.pow(10, (perBandLp[i] + aw[i]) / 10);
    }
    return aSum > 0 ? 10 * Math.log10(aSum) : -Infinity;
}
function energySumPerBand(perSource) {
    const n = perSource[0]?.perBandLp.length ?? 10;
    const out = new Float64Array(n);
    for (let i = 0; i < n; i++) {
        let acc = 0;
        for (const { perBandLp } of perSource)
            acc += Math.pow(10, perBandLp[i] / 10);
...
export async function snapshotProject(project, dem) {
    ...
    const pairs = new Map();
    const srcAbsAtSnapshot = new Map();
    const rxAbsAtSnapshot = new Map();
    const results = project.receivers.map((rx) => {
        ...
        for (const src of project.sources) {
            const { snapshot, srcAbsXyz } = snapshotPair(src, rx.latLng, rx.heightAboveGroundM, project, barriersFlat, dem, origin);
...
export function extrapolateProject(project, snapshot) {
    const aw = aWeights(project.scenario.bandSystem);
    ...
    const results = project.receivers.map((rx) => {
        ...
        for (const src of project.sources) {
            ...
            if (!cached || !here)
                continue;
            const { lp, stale: pairStale } = extrapolateLpClamped(cached.snapshot, cached.srcAbsXyz, here, capPerBand);
            if (pairStale)
                stale = true;
...
export async function evaluateProject(project, dem) {
    return (await snapshotProject(project, dem)).results;
}
/// Exact grid evaluation that also captures per-cell-per-source gradients.
export async function snapshotGrid(project, dem, spacingM, rxHeightAboveGround) {
    await ensureSolverReady();
    const t0 = performance.now();
    const ca = project.calculationArea;
    if (!ca)
        throw new Error('calculationArea not set; cannot compute grid');
    ...
    const sw = [origin[0] - dLat, origin[1] - dLng];
    const ne = [origin[0] + dLat, origin[1] + dLng];
    const barriersFlat = packBarriers(project.barriers, origin);
    const g = project.settings?.ground.defaultG ?? 0.5;
    const sources = project.sources;
    const sourceIds = sources.map((s) => s.id);
    const srcAbsAtSnapshot = new Float32Array(sources.length * 3);
    const srcLocal = sources.map((s) => latLngToLocalMetres(s.latLng, origin));
        ...
        for (let col = 0; col < cols; col++) {
            ...
            for (let si = 0; si < sources.length; si++) {
                ...
                const snap = src.kind === 'wtg'
                    ? evaluate_wtg_with_grad_src_octave(lw, se, sn, srcZ[si], e, n, rxZ, g, barriersFlat, entry.rotorDiameterM ?? 120, false)
                    : evaluate_general_with_grad_src_octave(lw, se, sn, srcZ[si], e, n, rxZ, g, barriersFlat);
...
export function extrapolateGrid(project, snapshot, dem) {
    const t0 = performance.now();
    const aw = aWeights(project.scenario.bandSystem);
    ...
    for (let cellIdx = 0; cellIdx < cellCount; cellIdx++) {
        ...
        if (Math.abs(totalNew - totalBaseline) > capTotal)
            stale = true;
...
export async function evaluateGrid(project, dem, spacingM, rxHeightAboveGround) {
    await ensureSolverReady();
    const t0 = performance.now();
    const ca = project.calculationArea;
    if (!ca)
