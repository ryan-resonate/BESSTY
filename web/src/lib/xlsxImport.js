...
function isThirdOctave(frequencies) {
    ...
    for (let i = 1; i < fs.length; i++)
        if (fs[i - 1] > 0)
            ratios.push(fs[i] / fs[i - 1]);
    if (ratios.length === 0)
        return false;
    ratios.sort((a, b) => a - b);
    return ratios[Math.floor(ratios.length / 2)] < 1.5;
}
function extractRotorDiameter(name) {
...
function parseWtgWorkbook(wb, fname) {
    let entry = null;
    for (const sheetName of wb.SheetNames) {
        const ws = wb.Sheets[sheetName];
        const aoa = XLSX.utils.sheet_to_json(ws, { header: 1, defval: null });
        const r = (row, col) => aoa[row - 1]?.[col - 1] ?? null;
        if (asString(r(1, 1)) !== 'Model:' || asString(r(2, 1)) !== 'Mode:')
            continue;
        ...
        for (const w of windSpeeds)
            spectra[String(Number.isInteger(w) ? w : w)] = [];
        ...
        const bandSystem = isThirdOctave(frequencies) ? 'oneThirdOctave' : 'octave';
        const modeData = {
...
function parseFlatWorkbook(wb, fname) {
    ...
    for (let r = 1; r < aoa.length; r++) {
        ...
        if (!entry) {
            ...
            if (kind === 'auxiliary')
                entry.auxiliaryType = typeStr.toLowerCase();
...
export async function parseCatalogXlsx(file) {
    ...
    if (a1 === 'Model:')
        return parseWtgWorkbook(wb, file.name);
    if (a1 === 'Model' && asString(b1).toLowerCase() === 'mode')
        return parseFlatWorkbook(wb, file.name);
