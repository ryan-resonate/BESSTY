...
export async function parseDemGeoTiff(file, opts = {}) {
    ...
    if (!isSupportedEpsg(epsg)) {
        ...
        throw new Error(`GeoTIFF is in EPSG:${epsg}${known ? ` (${known.label})` : ''}; this CRS isn't registered. ` +
...
export async function inferGeoTiffCrs(file) {
    const buf = await file.arrayBuffer();
    const tiff = await fromArrayBuffer(buf);
    const image = await tiff.getImage();
