...
Named iso-lines at levels you choose — a night limit, a boundary criterion — each with its own colour, width and dash. Eight preset colours cover most cases in one click; **…** opens the full colour picker. Line width is capped between 0.5 and 12 px, and the dash pattern scales with it so a heavy line still reads as dashed. They are traced in the same pass as the stepped contours, so a level off the step grid (37.5 dB) costs nothing extra and does not add a step at that level.

Custom lines are compliance artefacts rather than presentation, so they keep drawing when the contour grid is switched off, and they carry their name rather than their level as the label. Tick **Export** to include a line in the KML, shapefile and PDF; in the shapefile the name lands in a `LABEL` attribute beside `THRESH_DB`, so stepped contours and named lines can be told apart downstream.
