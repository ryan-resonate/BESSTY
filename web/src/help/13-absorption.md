...
**A surface never screens its own reflection.** ISO 9613-2 section 7.5.2 scores a reflected ray along its bent physical path, which touches the reflecting surface at the bounce and never crosses it — so the reflector contributes its absorption loss to its own ray and nothing else (ISO/TR 17534-3's reflecting-barrier case T19 lists no barrier attenuation for the reflected ray). BESSTY's solver implements exactly that, for walls and for container bodies alike: only obstructions standing on the real bounce-to-receiver leg screen a reflection, while the bounce itself — and, for a container, the box's own other faces — cannot. A different wall, or a different part of the same wall such as the other arm of an L, still screens. Verified against T19's reference value with the barrier acting as both screen and reflector, and against mirrored-source hand calculations for both a wall and a container.
